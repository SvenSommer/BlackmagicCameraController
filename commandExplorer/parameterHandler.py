...
class ParameterHandler:
    ...
    def printMinAndMax(self):
        ...
        if self.parameter['type'] in "fixed16":
            ...
            print("{0}: {1} ({2})".format("Minimum",min ,self.parameter["minimum"]))
            print("{0}: {1} ({2})".format("Maximum",max ,self.parameter["maximum"]))
        else:
    ...
    def printParamInfo(self):
        if len(self.parameter['index']) == 0:
            print("Parameter has to be an {0} between:".format(self.parameter['type']))
        else:
            print("Parameter consists of {0} sub parameters. These are '{1}'".format(len(self.parameter['index']),"', '".join(self.parameter['index']) ))  
        self.printMinAndMax()
        print("Interpretation: {0}".format(self.parameter['interpretation']))
        
    def checkParameter(self, parameter_value):
        if "minimum" in self.parameter:
            if parameter_value < self.parameter['minimum']:
                _ = input("ERROR: Value to low. Minumum value is {0}".format(self.parameter['minimum']))
                return False
        if "maximum" in self.parameter:
            if parameter_value > self.parameter['maximum']:
                _ = input("ERROR: Value to high. Maximum value is {0}".format(self.parameter['maximum']))
                return False
        return True
    ...
    def getParameterInput(self, command:Command, name):
        try:
            if self.parameter['type'] in self.integer_types:
                parameter_value = int(input("Please specify parameter {0}:".format(name)))
            elif self.parameter['type'] in "fixed16":
                parameter_value = self.to_float(int(input("Please specify parameter {0}:".format(name))), 16)
            elif self.parameter['type'] in "string":
                parameter_value = str(input("Please specify parameter {0}:".format(name)))
            elif self.parameter['type'] in "boolean":
                user_input = str(input("Please specify parameter {0} ([true] or [false]):".format(name)))
                ...
                user_input = int(input("Please specify parameter {0}([0] or [1]):".format(name)))
                if user_input not in [0,1]:
                    print("ERROR: Your input is not a allowed value")
                    self.handleParam(command)
                parameter_value = user_input
            elif self.parameter['type'] in "void":
                parameter_value = int(input("Do you want to send this command? Type '1' an hit Enter!"))
            else:
                print("Error: unknown parametertype: {0}".format(self.parameter['type']))
                exit(1)
    ...
    def handleParam(self, command: Command) -> Command:
        self.printParamInfo()
        if len(self.parameter['index']) == 0:
            parameter_value = self.getParameterInput(command, "'{0}'".format(self.parameter['parameter']))
            if self.checkParameter(parameter_value):
                command.setValue(parameter_value)
                
            else:
                self.handleParam(command)
        else:
            for subparam in self.parameter['index']:
                parameter_subvalue = self.getParameterInput(command, "'{0}' from '{1}'".format(subparam, self.parameter['parameter']))
                if self.checkParameter(parameter_subvalue):
                    command.addValue(parameter_subvalue)
                else:
                    self.handleParam(command)
